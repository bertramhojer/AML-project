...
wandb.init(project="AML-exam", name=f'GCN_Sbert_2')

def count_parameters(model):
    return sum(p.numel() for p in model.parameters() if p.requires_grad)
def find_saved_model(path, pattern='gcn_modelembedding*layer.pt'):
    ...
    for epoch in range(1, 1 + epochs):
        ...
        if epoch >= 10 and epoch % eval_steps == 0:
            ...
            torch.save({    
                ...
                }, f'../data/checkpoint/gcn_model_embedding_{num_layers}layer_{epoch=}.pt')
...
torch.save({    
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            }, f'../data/gcn_model_embedding_{num_layers}layer.pt')
